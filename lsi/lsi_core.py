...
#        cls.EUDEM_PATH = cls.GLOBAL_DIR / "EUDEM_v2" / "eudem_dem_3035_europe.tif"
        ...
        cls.ELSUS_PATH = cls.GLOBAL_DIR / "ELSUSv2" / "ELSUSv2_six_datasets" / "elsus_v2_2.tif"
        cls.WEIGHTS_GLOBAL_PATH = cls.GLOBAL_DIR / "ELSUSv2" / "Weights_Outside" # This is used as root folder for Weights Outside of Europe
        cls.WEIGHTS_EUROPE_PATH = cls.GLOBAL_DIR / "ELSUSv2" / "DBF_weights" # This is used as root folder for Weights in Europe
        cls.LITHOLOGY_PATH = cls.GLOBAL_DIR / "LiMW_GIS 2015" / "LiMW_GIS_2015.gdb"
        cls.ELSUS_ZONES_PATH = cls.GLOBAL_DIR / "ELSUSv2" / "ELSUSv2_six_datasets" / "climate_phys_regions.shp"
@unique
...
def reclass_landcover(landcover, landcover_name):
    ...
    if landcover_name == LandcoverType.ESAWC.value:
        ...
        landcover_reclass = xr.where(landcover == 10, 3, landcover) # Tree cover -> Forest
        ...
        landcover_reclass = xr.where(landcover == 111, 5, landcover) # Artificial -> Urban areas
        landcover_reclass = xr.where(landcover_reclass == 112, 5, landcover_reclass) # Artificial -> Urban areas
        landcover_reclass = xr.where(landcover_reclass == 121, 5, landcover_reclass) # Artificial -> Urban areas
        landcover_reclass = xr.where(landcover_reclass == 122, 5, landcover_reclass) # Artificial -> Urban areas
        landcover_reclass = xr.where(landcover_reclass == 123, 5, landcover_reclass) # Artificial -> Urban areas
        landcover_reclass = xr.where(landcover_reclass == 124, 5, landcover_reclass) # Artificial -> Urban areas
        landcover_reclass = xr.where(landcover_reclass == 131, 5, landcover_reclass) # Artificial -> Urban areas
        landcover_reclass = xr.where(landcover_reclass == 132, 5, landcover_reclass) # Artificial -> Urban areas
        landcover_reclass = xr.where(landcover_reclass == 133, 5, landcover_reclass) # Artificial -> Urban areas
        landcover_reclass = xr.where(landcover_reclass == 141, 5, landcover_reclass) # Artificial -> Urban areas
        landcover_reclass = xr.where(landcover_reclass == 142, 5, landcover_reclass) # Artificial -> Urban areas
        landcover_reclass = xr.where(landcover_reclass == 211, 1, landcover_reclass) # Cropland -> Arable Land
        landcover_reclass = xr.where(landcover_reclass == 212, 1, landcover_reclass) # Cropland -> Arable Land
        landcover_reclass = xr.where(landcover_reclass == 213, 1, landcover_reclass) # Cropland -> Arable Land
        landcover_reclass = xr.where(landcover_reclass == 221, 1, landcover_reclass) # Cropland -> Arable Land
        landcover_reclass = xr.where(landcover_reclass == 222, 1, landcover_reclass) # Cropland -> Arable Land
        landcover_reclass = xr.where(landcover_reclass == 223, 1, landcover_reclass) # Cropland -> Arable Land
        landcover_reclass = xr.where(landcover_reclass == 231, 2, landcover_reclass) # Pasture/Meadow -> Grassland
        landcover_reclass = xr.where(landcover_reclass == 241, 1, landcover_reclass) # Cropland -> Arable Land
        landcover_reclass = xr.where(landcover_reclass == 242, 1, landcover_reclass) # Cropland -> Arable Land
        landcover_reclass = xr.where(landcover_reclass == 243, 1, landcover_reclass) # Cropland -> Arable Land
        landcover_reclass = xr.where(landcover_reclass == 244, 3, landcover_reclass) # Open Forest -> Forest
        landcover_reclass = xr.where(landcover_reclass == 311, 3, landcover_reclass) # Closed Forest -> Forest 
        landcover_reclass = xr.where(landcover_reclass == 312, 3, landcover_reclass) # Closed Forest -> Forest
        landcover_reclass = xr.where(landcover_reclass == 313, 3, landcover_reclass) # Closed Forest -> Forest
        landcover_reclass = xr.where(landcover_reclass == 321, 2, landcover_reclass) # Pasture/Meadow -> Grassland
        landcover_reclass = xr.where(landcover_reclass == 322, 2, landcover_reclass) # Shrub -> Grassland
        landcover_reclass = xr.where(landcover_reclass == 323, 2, landcover_reclass) # Shrub -> Grassland
        landcover_reclass = xr.where(landcover_reclass == 324, 3, landcover_reclass) # Open Forest -> Forest
        landcover_reclass = xr.where(landcover_reclass == 331, 4, landcover_reclass) # Bare -> Bare
        landcover_reclass = xr.where(landcover_reclass == 332, 4, landcover_reclass) # Bare -> Bare
        landcover_reclass = xr.where(landcover_reclass == 333, 4, landcover_reclass) # Bare -> Bare
        landcover_reclass = xr.where(landcover_reclass == 334, 4, landcover_reclass) # Bare -> Bare
        landcover_reclass = xr.where(landcover_reclass == 335, 997, landcover_reclass) # Not applicable -> Not applicable
        landcover_reclass = xr.where(landcover_reclass == 411, 997, landcover_reclass) # Not applicable -> Not applicable
        landcover_reclass = xr.where(landcover_reclass == 412, 997, landcover_reclass) # Not applicable -> Not applicable
        landcover_reclass = xr.where(landcover_reclass == 421, 997, landcover_reclass) # Not applicable -> Not applicable
        landcover_reclass = xr.where(landcover_reclass == 422, 997, landcover_reclass) # Not applicable -> Not applicable
        landcover_reclass = xr.where(landcover_reclass == 423, 997, landcover_reclass) # Not applicable -> Not applicable
        landcover_reclass = xr.where(landcover_reclass == 511, 6, landcover_reclass) # Not applicable -> Water areas
        landcover_reclass = xr.where(landcover_reclass == 512, 6, landcover_reclass) # Not applicable -> Water areas
        landcover_reclass = xr.where(landcover_reclass == 521, 6, landcover_reclass) # Not applicable -> Water areas
        landcover_reclass = xr.where(landcover_reclass == 522, 6, landcover_reclass) # Not applicable -> Water areas
        landcover_reclass = xr.where(landcover_reclass == 523, 6, landcover_reclass) # Not applicable -> Water areas
        landcover_reclass = xr.where(landcover_reclass == 999, 997, landcover_reclass) # Not applicable -> Not applicable
...
def reclass_landcover_elsus(landcover, proj_crs, landcover_name):
    ...
    if landcover_name == LandcoverType.ESAWC.value:
        # Reclassification of LULC for ESA WorldCover
        landcover_reclass = xr.where(landcover == 10, 3, landcover) # Tree cover -> Closed Forest
        landcover_reclass = xr.where(landcover_reclass == 20, 4, landcover_reclass) # Shrubland -> Shrub
        landcover_reclass = xr.where(landcover_reclass == 30, 5, landcover_reclass) # Grassland -> Pasture/Meadow
        landcover_reclass = xr.where(landcover_reclass == 40, 1, landcover_reclass) # Cropland -> Cropland
        landcover_reclass = xr.where(landcover_reclass == 50, 7, landcover_reclass) # Built-up -> Artificial
        landcover_reclass = xr.where(landcover_reclass == 60, 6, landcover_reclass) # Bare/Sparse vegetation -> Bare
        landcover_reclass = xr.where(landcover_reclass == 70, 997, landcover_reclass) # Snow and ice -> Not applicable
        landcover_reclass = xr.where(landcover_reclass == 80, 997, landcover_reclass) # Permanent water bodies -> Not applicable
        landcover_reclass = xr.where(landcover_reclass == 90, 997, landcover_reclass) # Herbaceous wetland -> Not applicable
        landcover_reclass = xr.where(landcover_reclass == 95, 997, landcover_reclass) # Mangroves -> Not applicable
        landcover_reclass = xr.where(landcover_reclass == 100, 5, landcover_reclass) # Moss and lichen -> Pasture/Meadow
...
def check_parameters(input_dict: dir) -> None:
    ...
    if (location == LocationType.GLOBAL.value) and (landcover_name is LandcoverType.CLC.value):
        raise ValueError(f"Corine Land Cover can not be used for GLOBAL calculations, only in Europe !")
...
def landcover_raster(landuse_dbf, lulc_path, landcover_name, aoi, proj_crs, output_resolution, output_path):
    """
    """
    LOGGER.info(
    ...
    if not os.path.exists(os.path.join(output_path, "landcover_weight.tif")):
        ...
        landcover_gdf = xr_to_gdf(landcover_reclass, lulc.rio.crs, landcover_reclass.name, "Value")
        ...
        lulc_tif = lulc_tif.rio.reproject(proj_crs, resolution = output_resolution, resampling=Resampling.bilinear)
...
def elevation_raster(elevation_dbf, dem_b, aoi, proj_crs, output_path):
    ...
    if not os.path.exists(os.path.join(output_path, "elevation_weight.tif")):
        ...
        elevation_d = dem_b.copy(data=elevation_arr).astype(np.float32).rename(elevation_name)
...
def hydro_raster(hydro_dbf, dem_buff, aoi, proj_crs, ref_raster, output_resolution, tmp_dir, output_path):
    """
    ...
    if not os.path.exists(os.path.join(output_path, "hydro_weight.tif")):
        ...
        dem_b = dem_b.rio.write_crs(dem_buff.rio.crs)
            # reproject
        ...
        LOGGER.info(
            "-- -- Preprocessing the DEM: Filling Pits"
        )
        # -- Fill pits
        ...
        LOGGER.info(
            "-- -- Preprocessing the DEM: Filling Depressions"
        )
        # -- Fill depressions
        ...
        hydro_tif = hydro_tif.rio.reproject(proj_crs,
...
def landcover_raster_eu(landcover_weight_path, landcover_path, reference_raster,
                         aoi_zone, proj_crs, zone_id, counter, final_weight_dbf, tmp_dir, output_resolution
                         ,landcover_name):
    """
    ...
    landcover_weight_dbf.loc[len(landcover_weight_dbf)] = [997,"Not Applicable", 0.0, None]
    ...
    try:
        ...
        raise ValueError(
            "Your AOI doesn't cover your Landcover layer."
        )
    ...
    final_weight_factor = final_weight_dbf[final_weight_dbf.Factor == "Landcover"][zone_class].iloc[0]
    landcover_tif = landcover_tif * final_weight_factor
    
    # Write in memory
    temp_dir = os.path.join(tmp_dir, AnyPath("landcover_" + str(zone_id) + "_" + str(counter) +"_utm.tif"))
    output_dir = os.path.join(tmp_dir, AnyPath("landcover_" + str(zone_id) + "_" + str(counter) +".tif"))
    
    rasters.write(landcover_tif, temp_dir, compress="deflate", predictor=1)
    rasters.write(landcover_tif.rio.reproject(proj_crs, resolution=output_resolution, resampling = Resampling.bilinear), output_dir, predictor=1)
    # From UTM to LatLon
...
def slope_raster_eu(slope_weight_path, dem, aoi_zone, proj_crs,
                     zone_id, counter, final_weight_dbf, tmp_dir, output_resolution):
    ...
    # -- Apply Final Weights
    zone_class = 'Z' + str(zone_id) # Class for the zone
    final_weight_factor = final_weight_dbf[final_weight_dbf.Factor == "Slope"][zone_class].iloc[0]
    # Apply factor
    slope_tif = slope_tif * final_weight_factor
    
    # Write in memory
    temp_dir = os.path.join(tmp_dir, AnyPath("slope_" + str(zone_id) + "_" + str(counter) + "utm.tif"))
    output_dir = os.path.join(tmp_dir, AnyPath("slope_" + str(zone_id) + "_" + str(counter) +".tif"))
    
    rasters.write(slope_tif, temp_dir, compress="deflate", predictor=1)
    rasters.write(slope_tif.rio.reproject(proj_crs, resolution = output_resolution, resampling = Resampling.bilinear), output_dir, predictor=1)
...
#    weights_path = input_dict.get(InputParameters.WEIGHTS_PATH.value) #not anymore
    ...
    try:
            if np.unique(dem_b) == 0 or np.unique(dem_b) == np.nan:
                raise ValueError(f'Your DEM has no elevations. Make sure your input DEM is valid')
    except:
    ...
    dem_b = dem_b.rio.reproject(proj_crs, resolution=output_resolution, resampling=Resampling.bilinear)
    ...
    if output_resolution:
        ...
        x_res, y_res = dem_b.rio.resolution()
        output_resolution = (abs(x_res) + abs(y_res)) / 2
    ...
    if location == LocationType.EUROPE.value:
        elsus_weights_path = DataPath.WEIGHTS_EUROPE_PATH # The path for each Zone weights will later
                                                          # created based on this path
        elsus_final_weights_dbf_path = str(DataPath.WEIGHTS_EUROPE_PATH / "FinalWeigths.dbf")
        
        geology_dbf_path = str(DataPath.WEIGHTS_GLOBAL_PATH / "Geology.dbf")
        global_final_weights_dbf_path = str(DataPath.WEIGHTS_GLOBAL_PATH / "Final_weights.dbf")
    ...
    if location == LocationType.EUROPE.value:
            ...
            LOGGER.info(
                "-- LSI - CRS: " + str(proj_crs) + " | resolution: " + str(output_resolution)
            )

            if europe_method == EUMethodType.FAST.value:
                LOGGER.info(
                    "-- LSI -- Method: Europe fast method based on ELSUS layer"
                )
                ...
                lsi_tif = lsi_tif.rio.reproject(proj_crs, resolution=output_resolution, resampling=Resampling.bilinear)
                lsi_tif. rasters.crop(lsi_tif, aoi)
                lsi_tif.name = 'Value'

                LOGGER.info(
                    "-- Writing lsi.tif in memory"
                )
            ...
            geology_tif = geology_raster(geology_dbf, litho_shp, dem, aoi, proj_crs, output_path)
            
            # -- 2. Landcover + Slope (calculations per zone in the AOI according to ELSUS)

            LOGGER.info(
                "-- Defining physio zones for Europe Refined method"
            )
            ...
            zone_to_dbf = { 0: 'Zone0',  1: 'Zone1',  2: 'Zone2', 3: 'Zone3',
                            4: 'Zone4',  5: 'Zone5',  6: 'Zone6',}

            LOGGER.info(
                str("-- Produce the Land use and Slope raster for the LSI model | Amount of sub-zones to be processed: " + str(physio_zones_aoi["Zone"].count()))
            )
            ...
            for index, row in physio_zones_aoi.iterrows():
                ...
                if zone != 0: # For Zone0 the Landcover is not used
                    ...
                    landcover_w_path = str(elsus_weights_path /  str(str(db_file) + "/Landcover_Z" + str(zone) + ".dbf"))
                else:
                    ...
                    landcover_w_path = str(elsus_weights_path / "Zone1/Landcover_Z1.dbf") # A random file is chosen. At the end
                                                                                                # the weights are set to 0 for Zone0
                # -- Compute the Rasters
                landcover_dir =  landcover_raster_eu(landcover_w_path, lulc_path, dem, zone_geom, proj_crs, zone, i, fw_dbf, tmp_dir, output_resolution, landcover_name)
                landcover_list.append(landcover_dir)   

                # ---- SLOPE and LITHOLOGY ----
                # -- Define weights path
                #slope_w_path = os.path.join(elsus_weights_path,  str(db_file) + "/Slope_Z" + str(zone) + ".dbf")
                slope_w_path = str(elsus_weights_path / str(str(db_file) + "/Slope_Z" + str(zone) + ".dbf"))
                # -- Compute the Rasters
                slope_dir =  slope_raster_eu(slope_w_path, dem_b, zone_geom, proj_crs, zone, i, fw_dbf, tmp_dir, output_resolution)

                slope_list.append(slope_dir)   
            
            # Mosaicing the rasters by zone into the complete Raster
            slope_mosaic_path = mosaicing(slope_list, proj_crs, output_path, "slope_weight.tif")
            landcover_mosaic_path = mosaicing(landcover_list, proj_crs, output_path, "landcover_weight.tif")
            ...
            geology_tif = rasters.collocate(slope_tif, geology_tif, Resampling.bilinear) # Collocate Geology for LSI computation
            ...
            lsi_tif = (   slope_tif
                        + geology_tif*float(geology_weights) 
                        + landcover_tif
                        )
        ...
        #    warnings.simplefilter("ignore") # the buffer is being applied in degrees, not meters
        dem = rasters.crop(dem_b, aoi)
        dem = dem.rio.reproject(proj_crs,
                                resolution=output_resolution,
                                resampling=Resampling.bilinear)
        ...
        # momentaneous line to add Not Applicable class
        geology_dbf.loc[len(geology_dbf)] = ["Not Applicable", 997, 0.0, 0.0, None]
        geology_layer = geology_raster(geology_dbf, litho_shp, dem, aoi, proj_crs, output_path)
        ...
        # Landcover
        landuse_layer = landcover_raster(landuse_dbf, lulc_path, landcover_name, aoi, proj_crs, output_resolution, output_path)

        # -- 4. Elevation
        elevation_dbf = gpd.read_file(elevation_dbf_path)
        elevation_layer = elevation_raster(elevation_dbf, dem_b, aoi, proj_crs, output_path)

        # -- 5. Hydro
        hydro_dbf = gpd.read_file(hydro_dbf_path)
        hydro_layer = hydro_raster(hydro_dbf, dem_b, aoi, proj_crs, elevation_layer, output_resolution, tmp_dir, output_path)
        ...
        elevation_layer = rasters.read(os.path.join(output_path, "elevation_weight.tif"))
        ...
        geology_layer = rasters.collocate(slope_layer, geology_layer, Resampling.bilinear)
        aspect_layer = rasters.collocate(slope_layer, aspect_layer, Resampling.bilinear)
        aspect_layer = rasters.crop(aspect_layer, aoi)
        landuse_layer = rasters.collocate(slope_layer, landuse_layer, Resampling.bilinear)
    ...
    lsi_tif = rasters.crop(lsi_tif, aoi)   
    lsi_tif  = xr.where(lsi_tif > 10, np.nan, lsi_tif) # There should not be values greater than 10 (border effect)
    lsi_tif  = xr.where(lsi_tif < 0, np.nan, lsi_tif) # There should not be negative values (border effect)
    lsi_tif = lsi_tif.rio.write_crs(proj_crs, inplace=True)
